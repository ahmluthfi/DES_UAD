...
class DataSourceImpl implements DataSource {
  ...
  Future<AkademikStudentStatus> getStudentStatus() async {
    try {
      final Response response = await get(Uri.parse('$url${endpoint['akademik_student_status']}'));
      if (response.statusCode == 200) {
        print('Response Body: ${response.body}');
        return akademikStudentStatusFromJson(response.body);
      } else {
        throw ServerException(message: 'Gagal Mengambil Data: ${response.statusCode}');
      }
    } catch (e) {
      throw ServerException(message: e.toString());
    }
